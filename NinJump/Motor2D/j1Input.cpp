...
{
	BROFILER_CATEGORY("Input Awake", Profiler::Color::Gainsboro);
...
bool j1Input::Start()
{
	BROFILER_CATEGORY("Input Start", Profiler::Color::GhostWhite);
	SDL_StopTextInput();
	return true;
}
...
{
	BROFILER_CATEGORY("Input PreUpdate", Profiler::Color::GoldenRod);
...
{
	BROFILER_CATEGORY("Input CleanUp", Profiler::Color::Gray);
...
iPoint j1Input::GetActualMousePosition()
{
	iPoint ret; 
	ret.x = mouse_x; 
	ret.y = mouse_y; 
